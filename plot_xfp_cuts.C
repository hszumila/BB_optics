 ...
 //
  ...
  for  (Int_t nf=0;nf<NumFoil;nf++) {
	...
	if (xpfp_xfp_cut[nf][nd][nc]) {
      ...
      for (Int_t nys=0;nys<13;nys++) { xs_line[nys]->Draw();}
      for (Int_t nys=0;nys<13;nys++) { xs_text[nys]->Draw();}
      canCut[nf][nd]->cd(3);
      gPad->SetLogz();    
      for  (Int_t ny=0;ny<13;ny++) {
	if (hYsXs_DelCut_XpXfpCut[nf][nd][ny]->Integral()>0) {
      ...
      for (Int_t nys=0;nys<13;nys++) { xs_line[nys]->Draw("same");}
      for (Int_t nys=0;nys<13;nys++) { xs_text[nys]->Draw("same");}
      TString end = ".pdf";
