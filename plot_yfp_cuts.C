...
void plot_yfp_cuts(Int_t nrun=1814,Int_t FileID=-2) {
 ...
 //
  ...
  //
	...
	for  (Int_t nd=0;nd<ndelcut;nd++) {
	  hYsXs_DelCut_YpYfpCut[nf][nd].resize(7);
	}
	}
	for  (Int_t nc=0;nc<NumFoil;nc++) {
	for  (Int_t nd=0;nd<ndelcut;nd++) {
	for  (Int_t ny=0;ny<7;ny++) {
   ...
   outCutFile=Form("cuts/YpFpYFp_%s_%d_cut.root",OpticsID.Data(),FileID);
	...
	gPad->SetLogz();    
	  ...
	  hYsXs_DelCut[nf][nd]->SetMinimum(1);
          for (Int_t nys=0;nys<7;nys++) { ys_line[nys]->Draw();}
          for (Int_t nys=0;nys<7;nys++) { ys_text[nys]->Draw();}
	  canCut[nf][nd]->cd(3);
	gPad->SetLogz();    
	  	for  (Int_t ny=0;ny<7;ny++) {
		  if (hYsXs_DelCut_YpYfpCut[nf][nd][ny]->Integral()>0) {
		    ...
		    for (Int_t nys=0;nys<7;nys++) { ys_line[nys]->Draw("same");}
		    for (Int_t nys=0;nys<7;nys++) { ys_text[nys]->Draw("same");}
	  TString end = ".pdf";
